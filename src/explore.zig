...
pub const FileOutline = struct {
    ...
    pub fn deinit(self: *FileOutline) void {
        ...
        if (self.parse_warning) |warning| self.allocator.free(warning);
    }
};
...
pub const SearchOptions = struct {
    ...
    path_set: ?*const std.StringHashMap(void) = null,
    compact: bool = false,
...
pub const Explorer = struct {
    ...
    fn parseOutlineWithTreeSitter(self: *Explorer, path: []const u8, content: []const u8) !FileOutline {
        ...
        if (tree_sitter_mod.nodeHasError(root)) {
            switch (grammar) {
                .rust => {},
        ...
        switch (grammar) {
            .c, .cpp => try self.parseCTreeSitterItems(&outline, content, root),
            .rust => try self.parseRustTreeSitterItems(&outline, content, root, false),
            .typescript, .tsx => {
                var i: u32 = 0;
            ...
            .python => try self.parsePythonTreeSitterItems(&outline, content, root, false),
            .go => try self.parseGoTreeSitterItems(&outline, content, root),
            .dart => try self.parseDartTreeSitterItems(&outline, content, root, false),
            .php => try self.parsePhpTreeSitterItems(&outline, content, root, false),
            .hcl => try self.parseHclTreeSitterItems(&outline, content, root),
            .r => try self.parseRTreeSitterItems(&outline, content, root),
            .ruby => try self.parseRubyTreeSitterItems(&outline, content, root),
        }
    ...
    fn parseOutlineWithParser(parser: *Explorer, path: []const u8, content: []const u8) !FileOutline {
        const parse_warning_msg = "tree-sitter parse error; outline generated by conservative fallback";
        var parse_warning: ?[]const u8 = null;
        if (build_options.tree_sitter) {
            if (parser.parseOutlineWithTreeSitter(path, content)) |ts_outline| {
                return ts_outline;
            } else |err| switch (err) {
                error.UnsupportedLanguage, error.IncompatibleLanguage => {},
                error.ParseFailed => parse_warning = parse_warning_msg,
                else => return err,
            }
        }
        ...
        errdefer outline.deinit();
        outline.byte_size = content.len;
        if (parse_warning) |warning| {
            outline.parse_warning = try parser.allocator.dupe(u8, warning);
        }
        ...
        while (lines.next()) |line| {
            ...
            if (outline.language == .typescript or outline.language == .javascript or
                ...
                outline.language == .zig or outline.language == .hcl or outline.language == .php or
                outline.language == .dart or outline.language == .java or
                outline.language == .kotlin or outline.language == .svelte or
                outline.language == .vue or outline.language == .astro or
                outline.language == .css or outline.language == .scss or
            ...
            if (outline.language == .typescript or outline.language == .javascript or
                outline.language == .c or outline.language == .cpp or outline.language == .php)
            {
                if (trimLineForFallbackStringState(trimmed, &in_string)) |line_for_parse| {
                    trimmed = line_for_parse;
    ...
    pub fn renderOutline(
        ...
        if (!compact) {
            if (outline.parse_warning) |warning| {
                w.print("  warning: {s}\n", .{warning}) catch {};
            }
        }
        for (outline.symbols.items) |sym| {
    ...
    pub fn cloneOutline(src: *const FileOutline, allocator: std.mem.Allocator) !FileOutline {
        ...
        if (src.parse_warning) |warning| {
            dst.parse_warning = try allocator.dupe(u8, warning);
        }
    ...
    pub fn searchContent(self: *Explorer, query: []const u8, allocator: std.mem.Allocator, max_results: usize) ![]const SearchResult {
        return self.searchContentWithOptions(query, allocator, .{ .max_results = max_results });
    }

    pub fn searchContentWithOptions(self: *Explorer, query: []const u8, allocator: std.mem.Allocator, options: SearchOptions) ![]const SearchResult {
        self.mu.lockShared();
        defer self.mu.unlockShared();
        ...
        if (word_hits.len > 0) {
            ...
            for (word_hits, 0..) |hit, ordinal| {
                ...
                if (!searchPathEligible(hit_path, options)) continue;
        ...
        if (result_list.items.len == 0 and query.len >= 3) {
            ...
            for (prefix_hits) |hit| {
                ...
                if (!searchLineEligible(hit_path, line_text, options)) continue;
        ...
        if (candidate_paths) |cp| {
            if (cp.len > 0) {
                ...
                const max_per_file = options.max_per_file orelse
                    @max(@as(usize, 1), max_results / @max(@as(usize, 1), estimated_total));
                for (cp) |path| {
                    ...
                    defer ref.deinit();
                    try searchInContentWithOptions(path, ref.data, query, allocator, max_per_file, options, &result_list);
                    if (result_list.items.len >= max_results) {
                        return self.rerankAndFinalize(&result_list, query, allocator);
                    }
                }
        ...
        if (result_list.items.len < max_results) {
            ...
            while (skip_iter.next()) |key_ptr| {
                ...
                if (!searchPathEligible(key_ptr.*, options)) continue;
        ...
        const trigram_authoritative = candidate_paths != null;
        const should_scan_remaining =
            result_list.items.len < max_results and
            options.reliability_fallback and
            !trigram_authoritative;
        if (should_scan_remaining) {
    ...
    fn sortSymbolLocations(locs: []SymbolLocation) void {
        if (locs.len <= 1) return;
        std.sort.block(SymbolLocation, locs, {}, struct {
            pub fn lessThan(_: void, a: SymbolLocation, b: SymbolLocation) bool {
                const ar = symbolLocationRank(a);
                const br = symbolLocationRank(b);
    ...
    fn symbolLocationRank(loc: SymbolLocation) u32 {
        var rank: u32 = switch (loc.kind) {
            .function, .method, .struct_def, .enum_def, .union_def, .trait_def, .type_alias, .class_def, .interface_def, .macro_def => 0,
        ...
        if (pathHasSegment(loc.path, "tests") or pathHasSegment(loc.path, "test")) rank += 25;
        if (pathHasSegment(loc.path, "examples") or pathHasSegment(loc.path, "example")) rank += 20;
        if (pathHasSegment(loc.path, "vendor") or pathHasSegment(loc.path, "node_modules") or pathHasSegment(loc.path, "third_party")) rank += 50;
        if (isDocLanguage(detectLanguage(loc.path))) rank += 40;
        if (std.mem.indexOf(u8, loc.path, "generated") != null or std.mem.indexOf(u8, loc.path, ".gen.") != null) rank += 35;
        return rank;
    }
    ...
    pub fn searchContentWithScope(self: *Explorer, query: []const u8, allocator: std.mem.Allocator, max_results: usize) ![]const ScopedSearchResult {
        return self.searchContentWithScopeOptions(query, allocator, .{ .max_results = max_results });
    }

    pub fn searchContentWithScopeOptions(self: *Explorer, query: []const u8, allocator: std.mem.Allocator, options: SearchOptions) ![]const ScopedSearchResult {
        const plain_results = try self.searchContentWithOptions(query, allocator, options);
        defer {
            for (plain_results) |r| {
                allocator.free(r.line_text);
                allocator.free(r.path);
...
fn searchPathEligible(path: []const u8, options: SearchOptions) bool {
    if (options.path_set) |set| {
        if (!set.contains(path)) return false;
    }
    if (options.path_glob) |glob| {
        if (!matchGlob(glob, path)) return false;
    }
    return true;
}

fn searchLineEligible(path: []const u8, line: []const u8, options: SearchOptions) bool {
    if (!searchPathEligible(path, options)) return false;
    if (options.compact and isCommentOrBlank(line, detectLanguage(path))) return false;
    return true;
}
...
fn appendTargetLineHitsWithOptions(
    path: []const u8,
    content: []const u8,
    allocator: std.mem.Allocator,
    target_lines: []const u32,
    ...
    if (target_lines.len == 0 or result_list.items.len >= options.max_results) return;
    result_list.ensureUnusedCapacity(allocator, @min(target_lines.len, options.max_results - result_list.items.len)) catch {};
    var target_i: usize = 0;
    var line_num: u32 = 0;
    var lines = std.mem.splitScalar(u8, content, '\n');
    while (lines.next()) |line| {
        ...
        if (target_lines[target_i] != line_num) continue;
        target_i += 1;
        if (!searchLineEligible(path, line, options)) continue;
        ...
        if (result_list.items.len >= options.max_results) return;
    }
}
...
fn searchInContent(path: []const u8, content: []const u8, query: []const u8, allocator: std.mem.Allocator, max_per_file: usize, max_results: usize, result_list: *std.ArrayList(SearchResult)) !void {
    try searchInContentWithOptions(path, content, query, allocator, max_per_file, .{ .max_results = max_results }, result_list);
}

fn searchInContentWithOptions(path: []const u8, content: []const u8, query: []const u8, allocator: std.mem.Allocator, max_per_file: usize, options: SearchOptions, result_list: *std.ArrayList(SearchResult)) !void {
    const max_results = options.max_results;
    if (query.len == 0 or content.len == 0 or max_per_file == 0 or max_results == 0 or result_list.items.len >= max_results) return;
    if (!searchPathEligible(path, options)) return;
        ...
        if (pos + VW <= end) {
            ...
            while (mask != 0) {
                ...
                if (matchAtCaseInsensitive(content, cand, query_lower)) {
                    ...
                    if (!searchLineEligible(path, line, options)) {
                        current_line += 1;
...
fn trimLineForFallbackStringState(line: []const u8, in_string: *u8) ?[]const u8 {
    if (in_string.* != 0) {
        if (findClosingQuote(line, in_string.*, 0)) |close_pos| {
            in_string.* = 0;
            return std.mem.trimStart(u8, line[close_pos + 1 ..], " \t");
        }
        return null;
    }
    ...
    while (i < line.len) : (i += 1) {
        const c = line[i];
        if (c == '/' and i + 1 < line.len and line[i + 1] == '/') return line;
        if (c != '"' and c != '\'') continue;
        if (findClosingQuote(line, c, i + 1)) |close_pos| {
            i = close_pos;
            continue;
        }
        in_string.* = c;
...
fn findClosingQuote(line: []const u8, quote: u8, start: usize) ?usize {
    var i = start;
    var escaped = false;
    while (i < line.len) : (i += 1) {
        const c = line[i];
